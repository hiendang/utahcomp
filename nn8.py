...
class AdjustVariable(object):
	def __init__(self, name, start=0.03, stop=0.001, duration=700):
		...
		self.ls = np.linspace(self.start, self.stop, self.duration)
...
class EarlyStopping(object):
	...
	def __call__(self, nn, train_history):
		...
		if self.verbose:			
			print "%04d   |   %5f   |   %5f   |   %5f   |  %5f  | %4.2f"%(current_epoch,train_history[-1]['train_loss'],valid_log_loss,valid_acc,valid_roc,train_history[-1]['dur'])
